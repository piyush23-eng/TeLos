...
const baseProblems: DrillProblem[] = [
  {
...
const additionalProblems: DrillProblem[] = [
...
function createDrillDetails(problem: { title: string; description: string; category: string; difficulty: string }) {
  ...
  return {
    ...
    output,
    constraints: [
      problem.difficulty === 'Hard' ? 'Design for input sizes up to 100,000 records or nodes.' : 'Design for input sizes up to 10,000 records or nodes.',
      'State time and space complexity before coding.',
      'Cover empty input, duplicate values or identifiers, and the smallest valid input.'
    ],
    examples: [example],
...
export const problemCatalog: DrillProblem[] = [...baseProblems, ...additionalProblems].map(problem => ({
  ...problem,
